...
Entries before the 2026-08-24 Codex port are upstream history. Paths such as
`OtherWorlds/Ultimate Websites/builds/`, `BUILD-REPORT.md`, and `lab/*.mjs`
refer to the original author's private build environment; they are archival
context, not files this standalone skill expects to find.

## 2026-08-24: keyless Codex-native media workflow

- Made the active workflow keyless: user-owned assets come first, native
  `image_gen` handles still generation and edits, and no project API key,
  provider SDK, credit probe, pricing table, or external generation client is
  part of the default route.
- Added a manifest-backed media contract. Generated outputs are inspected,
  copied out of transient Codex storage, versioned without overwrite, and
  recorded in each build's `ASSET-MANIFEST.md`.
- Kept supplied-footage grading, dense-GOP ffmpeg encoding, posters, portrait
  sources, and still-safe code choreography as first-class paths. Generated
  video now has an explicit capability gate instead of an implicit fallback.
- Added a dormant `gemini-ab` comparison contract. It has no bundled SDK,
  key, or model identifier and activates only after explicit user opt-in plus
  inspection of a currently callable connector. Video comparisons are labelled
  asymmetric because the native route does not generate video.
- Removed KIE-specific operational guidance from the active references and
  doctor path. Older entries below retain provider and billing names strictly
  as archival history.

## 2026-08-24: native Codex port

- Removed Claude Code's `allowed-tools` contract and shortened the discovery
  description for Codex skill selection.
- Added `agents/openai.yaml`, `$scrollcraft` invocation metadata, repo-local
  discovery support, Codex image inspection guidance, and approval boundaries
  around package installs, network access, paid generation, and publishing.
- Added cross-platform `scripts/encode.mjs`, quoted skill-root command paths,
  managed-session server instructions, and Windows `USERPROFILE` discovery for
  contact-sheet ffmpeg resolution.
- Bundled the upstream MIT license and fingerprint examples. The later keyless
  media conversion removed the temporary provider environment template.
