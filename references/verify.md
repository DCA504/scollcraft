...
Then **inspect `sheet.png` with `view_image`**. The whole point of shooting
contiguously is looking at the frames side by side; a folder of PNGs does not
...
```bash
node -e "fetch('http://localhost:45XX').then(r => r.text()).then(t => console.log(t.match(/<title>[^<]*<\/title>/i)?.[0] ?? 'no title')).catch(e => { console.error(e); process.exit(1); })"
node -e "fetch('http://localhost:45XX/assets/leg01.mp4', { method: 'HEAD' }).then(r => { console.log(r.status); if (!r.ok) process.exit(1); }).catch(e => { console.error(e); process.exit(1); })"
```
