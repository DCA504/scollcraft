...
[uniqueness.md](uniqueness.md). Short version:
...
- **Assets:** the first build with **zero** generated imagery. Uniqueness does
  not require generated media, and a page that proves it is worth having in the
  table.
...
- **Assets:** the first build to use a **seam-locked chain** (nine legs: seven
  chained between prescribed head and tail frames, two more chained on start
  images only, every joint out of the encoded file at 22-45 dB PSNR) and the
  first to derive its poster ladder from the encoded clips themselves. Chaining
  is now spent; the SKILL.md warning against it stands for everyone else.
